...
do
    ...
    python ../scripts/elf.py ./build/main ../src/ram ../scripts/vhdl_rams > ./build/ram.dump
    ...
    cd build
    echo "SIMULATING..."
    test_result=$(ghdl -r "riscv" --stop-time=8000us --wave=../tests/build/wave.ghw | grep -E "(report note)" | grep -Eo "[0-9]+$")
